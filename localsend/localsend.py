...
class CallbacksDict(TypedDict):
    ...
    prepare_upload: Callable[[Device, dict[str, FileInfo]], Awaitable[None]]
...
class LSHttpServer:
    ...
    async def _prepare_upload(self, request: Request) -> dict:
        data = await request.json()
        ...
        return await self._localsend.create_uploads(device, files)
...
class LocalSend:
    ...
    async def create_uploads(self, device: Device, files: dict[str, FileInfo]) -> dict:
        await self._callbacks["prepare_upload"](device, files)
        sess_id = str(uuid4())
        return {
    ...
    async def handle_upload(self, session_id: str, file_id: str, file_token: str, stream: AsyncGenerator) -> None:
        ...
        recv_path = Path("received") / session["id"] / file.file_name
    ...
    async def _prepare_upload_stub(self, device: Device, files: dict[str, FileInfo]) -> None:
        ...
